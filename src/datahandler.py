...
class DataHandler:
    ...
    def evaluate(self):
        if self.Y_predicted is None:
            ...
            return {"test_index_start": self.test_index_start, "test_index_end": self.test_index_end, "train_index_start": self.train_index_start, "train_index_end": self.train_index_end, "Y_test": self.Y_test, "Y_predicted": self.Y_predicted, "correct": len(self.Y_test)-len(wrong_ind), "wrong": len(wrong_ind), "wrong_ind": wrong_ind, "wrong_nos": wrong_nos, "wrong_orig_mnist": wrong_ind_orig_mnist, "runtime": self.timer.result(), "k-value": self.k, "filter": self.filter_value, "layers": self.layers}
